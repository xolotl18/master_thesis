...
class LearningToDownSample(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv = ConvBlock(in_channels=in_channels, out_channels=8, stride=2)
        self.dsconv1 = nn.Sequential(
            # depthwise convolution
            nn.Conv2d(8, 8, kernel_size=3, stride=2, padding=1, dilation=1, groups=8, bias=False),
            nn.BatchNorm2d(8),
            # pointwise convolution
            nn.Conv2d(8, 12, kernel_size=1, stride=1, padding=0, dilation=1, groups=1, bias=False),
            nn.BatchNorm2d(12),
            nn.ReLU(inplace=True))
        self.dsconv2 = nn.Sequential(
            nn.Conv2d(12, 12, kernel_size=3, stride=2, padding=1, dilation=1, groups=12, bias=False),
            nn.BatchNorm2d(12),
            nn.Conv2d(12, 16, kernel_size=1, stride=1, padding=0, dilation=1, groups=1, bias=False),
            nn.BatchNorm2d(16),
            nn.ReLU(inplace=True))
...
class GlobalFeatureExtractor(nn.Module):
    def __init__(self):
        ...
        self.first_block = nn.Sequential(Bottleneck(16, 16, 2, 2),
                                         Bottleneck(16, 16, 1, 2),
                                         Bottleneck(16, 16, 1, 2))
        self.second_block = nn.Sequential(Bottleneck(16, 24, 2, 2),
                                          Bottleneck(24, 24, 1, 2),
                                          Bottleneck(24, 24, 1, 2))
        self.third_block = nn.Sequential(Bottleneck(24, 32, 1, 2),
                                         Bottleneck(32, 32, 1, 2),
                                         Bottleneck(32, 32, 1, 2))
        self.ppm = PPMModule(32, 32)
...
class FeatureFusion(nn.Module):
    def __init__(self, scale_factor):
        ...
        self.conv_high_res = nn.Conv2d(16, 32, kernel_size=1, stride=1, padding=0, bias=True)

        self.dwconv = ConvBlock(in_channels=32, out_channels=32, stride=1, padding=scale_factor,
                                dilation=scale_factor, groups=32)
        self.conv_low_res = nn.Conv2d(32, 32, kernel_size=1, stride=1, padding=0, bias=True)
...
class Classifier(nn.Module):
    def __init__(self, num_classes, scale_factor):
        ...
        self.dsconv1 = nn.Sequential(
            # depthwise convolution
            nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1, dilation=1, groups=32, bias=False),
            nn.BatchNorm2d(32),
            # pointwise convolution
            nn.Conv2d(32, 32, kernel_size=1, stride=1, padding=0, dilation=1, groups=1, bias=False),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True))
        self.dsconv2 = nn.Sequential(
            nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1, dilation=1, groups=32, bias=False),
...
class PPMModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, sizes=(1, 2, 3, 6)):
        ...
        self.pool1 = nn.AvgPool2d(16)    # produces 1x1x32 feature map
        self.pool2 = nn.AvgPool2d(8)     # produces 2x2x32 feature map
        self.pool3 = nn.AvgPool2d(4)     # produces 4x4x32 feature map
        self.pool4 = nn.AvgPool2d(2)     # produces 8x8x32 feature map
