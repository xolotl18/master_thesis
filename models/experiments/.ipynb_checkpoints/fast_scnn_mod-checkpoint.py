...
class FastSCNN(nn.Module):
    def __init__(self, in_channels, num_classes, a=1, t=6, r=3, pp=True):
        ...
        self.learning_to_down_sample = LearningToDownSample(in_channels, a)
...
class LearningToDownSample(nn.Module):
    def __init__(self, in_channels, a):
        ...
        self.conv = ConvBlock(in_channels=in_channels, out_channels=int(32*a), stride=2)
        self.dsconv1 = nn.Sequential(
            # depthwise convolution
            nn.Conv2d(int(32*a), int(32*a), kernel_size=3, stride=2, 
                      padding=1, dilation=1, groups=int(32*a), bias=False),
            nn.BatchNorm2d(int(32*a)),
            # pointwise convolution
            nn.Conv2d(int(32*a), int(48*a), kernel_size=1, stride=1, 
                      padding=0, dilation=1, groups=1, bias=False),
            nn.BatchNorm2d(int(48*a)),
            nn.ReLU(inplace=True))
        self.dsconv2 = nn.Sequential(
            nn.Conv2d(int(48*a), int(48*a), kernel_size=3, stride=2, 
                      padding=1, dilation=1, groups=int(48*a), bias=False),
            nn.BatchNorm2d(int(48*a)),
            nn.Conv2d(int(48*a), int(64*a), kernel_size=1, stride=1, 
                      padding=0, dilation=1, groups=1, bias=False),
            nn.BatchNorm2d(int(64*a)),
            nn.ReLU(inplace=True))
...
class GlobalFeatureExtractor(nn.Module):
    def __init__(self, a, t, r, pp):
        super().__init__()
        self.pp = pp
        
        if r == 3:
            self.first_block = nn.Sequential(Bottleneck(int(64*a), int(64*a), 2, int(t)),
                                             Bottleneck(int(64*a), int(64*a), 1, int(t)),
                                             Bottleneck(int(64*a), int(64*a), 1, int(t)))
            self.second_block = nn.Sequential(Bottleneck(int(64*a), int(96*a), 2, int(t)),
                                              Bottleneck(int(96*a), int(96*a), 1, int(t)),
                                              Bottleneck(int(96*a), int(96*a), 1, int(t)))
            self.third_block = nn.Sequential(Bottleneck(int(96*a), int(128*a), 1, int(t)),
                                             Bottleneck(int(128*a), int(128*a), 1, int(t)),
                                             Bottleneck(int(128*a), int(128*a), 1, int(t)))
        elif r == 2:
            self.first_block = nn.Sequential(Bottleneck(int(64*a), int(64*a), 2, int(t)),
                                             Bottleneck(int(64*a), int(64*a), 1, int(t)))
            ...
            #the default is the standard fastsccn with 3 repetitions of the bottleneck blocks
            self.first_block = nn.Sequential(Bottleneck(int(64*a), int(64*a), 2, int(t)),
                                             Bottleneck(int(64*a), int(64*a), 1, int(t)),
                                             Bottleneck(int(64*a), int(64*a), 1, int(t)))
            self.second_block = nn.Sequential(Bottleneck(int(64*a), int(96*a), 2, int(t)),
        ...
        self.ppm = PPMModule(int(128*a), int(128*a))
...
class FeatureFusion(nn.Module):
    def __init__(self, a, scale_factor):
        ...
        self.conv_high_res = nn.Conv2d(int(64*a), int(128*a), kernel_size=1, stride=1, 
                                       padding=0, bias=True)
        self.dwconv = ConvBlock(in_channels=int(128*a), out_channels=int(128*a), stride=1, 
                                padding=scale_factor, dilation=scale_factor, groups=int(128*a))
        self.conv_low_res = nn.Conv2d(int(128*a), int(128*a), kernel_size=1, stride=1, 
                                      padding=0, bias=True)
...
class Classifier(nn.Module):
    def __init__(self, num_classes, a, scale_factor):
        super().__init__()

        self.scale_factor = scale_factor
        self.dsconv1 = nn.Sequential(
            # depthwise convolution
            nn.Conv2d(int(128*a), int(128*a), kernel_size=3, stride=1, 
                      padding=1, dilation=1, groups=int(128*a), bias=False),
            nn.BatchNorm2d(int(128*a)),
            # pointwise convolution
            nn.Conv2d(int(128*a), int(128*a), kernel_size=1, stride=1, 
                      padding=0, dilation=1, groups=1, bias=False),
            nn.BatchNorm2d(int(128*a)),
            nn.ReLU(inplace=True))
        self.dsconv2 = nn.Sequential(
        ...
        self.conv = nn.Conv2d(int(128*a), num_classes, kernel_size=1, stride=1, padding=0, bias=True)
