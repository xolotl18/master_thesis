...
class LearningToDownSample(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv = ConvBlock(in_channels=in_channels, out_channels=24, stride=2)
        self.dsconv1 = nn.Sequential(
            # depthwise convolution
            nn.Conv2d(24, 24, kernel_size=3, stride=2, padding=1, dilation=1, groups=24, bias=False),
            nn.BatchNorm2d(24),
            # pointwise convolution
            nn.Conv2d(24, 36, kernel_size=1, stride=1, padding=0, dilation=1, groups=1, bias=False),
            nn.BatchNorm2d(36),
            nn.ReLU(inplace=True))
        self.dsconv2 = nn.Sequential(
            nn.Conv2d(36, 36, kernel_size=3, stride=2, padding=1, dilation=1, groups=36, bias=False),
            nn.BatchNorm2d(36),
            nn.Conv2d(36, 48, kernel_size=1, stride=1, padding=0, dilation=1, groups=1, bias=False),
            nn.BatchNorm2d(48),
            nn.ReLU(inplace=True))
...
class GlobalFeatureExtractor(nn.Module):
    def __init__(self):
        ...
        self.first_block = nn.Sequential(Bottleneck(48, 48, 2, 6),
                                         Bottleneck(48, 48, 1, 6),
                                         Bottleneck(48, 48, 1, 6))
        self.second_block = nn.Sequential(Bottleneck(48, 72, 2, 6),
                                          Bottleneck(72, 72, 1, 6),
                                          Bottleneck(72, 72, 1, 6))
        self.third_block = nn.Sequential(Bottleneck(72, 96, 1, 6),
...
class FeatureFusion(nn.Module):
    def __init__(self, scale_factor):
        ...
        self.conv_high_res = nn.Conv2d(48, 96, kernel_size=1, stride=1, padding=0, bias=True)

        self.dwconv = ConvBlock(in_channels=96, out_channels=96, stride=1, padding=scale_factor,
                                dilation=scale_factor, groups=96)
        self.conv_low_res = nn.Conv2d(96, 96, kernel_size=1, stride=1, padding=0, bias=True)
...
class Classifier(nn.Module):
    def __init__(self, num_classes, scale_factor):
        ...
        self.dsconv1 = nn.Sequential(
            # depthwise convolution
            nn.Conv2d(96, 96, kernel_size=3, stride=1, padding=1, dilation=1, groups=96, bias=False),
            nn.BatchNorm2d(96),
            # pointwise convolution
            nn.Conv2d(96, 96, kernel_size=1, stride=1, padding=0, dilation=1, groups=1, bias=False),
            nn.BatchNorm2d(96),
            nn.ReLU(inplace=True))
        self.dsconv2 = nn.Sequential(
            nn.Conv2d(96, 96, kernel_size=3, stride=1, padding=1, dilation=1, groups=96, bias=False),
        ...
        self.conv = nn.Conv2d(96, num_classes, kernel_size=1, stride=1, padding=0, bias=True)
...
class PPMModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, sizes=(1, 2, 3, 6)):
        ...
        self.pool1 = nn.AvgPool2d(16)    # produces 1x1x96 feature map
        self.pool2 = nn.AvgPool2d(8)     # produces 2x2x96 feature map
        self.pool3 = nn.AvgPool2d(4)     # produces 4x4x96 feature map
        self.pool4 = nn.AvgPool2d(2)     # produces 8x8x96 feature map
